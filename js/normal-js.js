(function($){
	...
	$.fn.modal=function(options){//遮罩
		...
		var windowHeight=window.screen.height;//可见手机高度
	...
	$.fn.modifyPwd=function(options){
		var step2=0,step3=0;
		var oripwd="172193";
		opt = $.extend({}, $.fn.modifyPwd.defaults, options);
		clearInput($(this));
	...
	$.fn.forgetPwd=function(options){//忘记密码
		option = $.extend({}, $.fn.forgetPwd.defaults, options);
	...
	$.fn.register=function(opts){//会员注册
		...
		opt = $.extend({}, $.fn.register.defaults,opts);
	...
	$.fn.getSlide=function(options){//楼层，业态下拉
		...
		var oImg,nImg;
		oImg=$(kinds.slideCell).find("img").eq(0).attr("src");
	    nImg=$(kinds.slideCell).find("img").eq(0).attr("datasrc");
	   // alert(oImg+' '+nImg);
		$(kinds.slideCell).each(function(){
			...
			$(this).on("click",function(e){
				...
				$(kinds.slideCell).removeClass("text-white").find("img").attr({"src":oImg,"datasrc":nImg});
				
				var $datasrc=$(this).find("img").attr("datasrc");
				
				$(this).addClass("text-white");
				$(this).find("img").attr({"datasrc":oImg,"src":nImg});
				...
				if($(ohref).hasClass("split-kinds")){
					$(ohref).find(kinds.splitItem).removeClass("text-primary").children("img").hide();
					$(ohref).find(kinds.splitItem).each(function(){
						if($(this).hasClass(kinds.splitActive)){
							$(this).addClass("text-primary").children("img").show();
						}
						$(this).on("click",function(e){
							e.preventDefault();
							$(ohref).find(kinds.splitItem).removeClass("text-primary");
							$(ohref).find(kinds.splitItem).removeClass(kinds.splitActive).children("img").hide();
							$(this).addClass(""+kinds.splitActive+" text-primary").children("img").show();
						});
					});
				}
				...
				$(kinds.mask).on("click",function(e){
					...
					$(kinds.slideCell).removeClass("text-primary text-white").find("img").attr({"src":oImg,"datasrc":nImg});
